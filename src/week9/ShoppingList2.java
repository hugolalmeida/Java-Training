...
//////////////////////////////////////////////
/**
 * Asks the user to enter an item to be stored in the shoppingList and then 
adds
 * the item to the shoppingList ArrayList. The user may enter as many items 
as
 ...
 * 
 * @param sIn          - The scanner to allow to get user input from the 
console
 * @param shoppingList - The array list to store the items of the shopping 
list
...
public static int addItems(Scanner sIn, ArrayList<String> shoppingList) {
	// FIXME: implement the method
		...
		do {
			...
			if(input.isEmpty()) {
				...
				String items=input.substring(0, separator).trim();
				int itemNumber=0;
			
				try {
				itemNumber = Integer.parseInt(input.substring(separator + 1).trim());
				shoppingList.add(items + ":" + itemNumber);
			    value=value+1;
	...
	/**
	 * Asks the user to enter an item to be deleted from the shoppingList and 
	then
	 * deletes the item from the shoppingList ArrayList. The user may delete as 
	many
	 * items as they want, hitting ENTER to stop deleting items and return to
	 * main(). Each time the user deletes an item from the list, a message will 
	be
			...
			System.out.println("Delete an item from the list (or just hit 'ENTER' when done): ");
			 input = sIn.nextLine();
				...
				break;
				 ...
				 boolean delete=shoppingList.remove(input);
				if (delete==true) {
					value=value+1;
	...
	public static void showItems(ArrayList<String> shoppingList) {
		...
		if(input.contains(":")) { 
			...
			int separator=input.indexOf(":");
			String items=input.substring(0, separator);
			int itemNumber=0;
			itemNumber = Integer.parseInt(input.substring(separator + 1));
	...
	public static void sortItems(ArrayList<String> shoppingList) {
		...
		for(String sortNum:shoppingList) {
		String input=sortNum;
         if(input.contains(":")) { 
